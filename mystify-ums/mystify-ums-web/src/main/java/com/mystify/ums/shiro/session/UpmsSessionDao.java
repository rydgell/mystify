package com.mystify.ums.shiro.session;
...
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    // 会话key
    private final static String UMS_SHIRO_SESSION_ID = "ums-shiro-session-id";
    // 全局会话key
    ...
    protected Serializable doCreate(Session session) {
        ...
        RedisUtil.set(UMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(session), (int) session.getTimeout() / 1000);
    ...
    protected Session doReadSession(Serializable sessionId) {
        String session = RedisUtil.get(UMS_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    protected void doDelete(Session session) {
        ...
        if ("client".equals(upmsType)) {
            ...
            String code = RedisUtil.get(UMS_CLIENT_SESSION_ID + "_" + sessionId);
            Jedis jedis = RedisUtil.getJedis();
            jedis.del(UMS_CLIENT_SESSION_ID + "_" + sessionId);
            jedis.srem(UMS_CLIENT_SESSION_IDS + "_" + code, sessionId);
            jedis.close();
        }
        if ("server".equals(upmsType)) {
            // 当前全局会话code
            String code = RedisUtil.get(UMS_SERVER_SESSION_ID + "_" + sessionId);
            // 清除全局会话
            RedisUtil.remove(UMS_SERVER_SESSION_ID + "_" + sessionId);
            // 清除code校验值
            RedisUtil.remove(UMS_SERVER_CODE + "_" + code);
            // 清除所有局部会话
            Jedis jedis = RedisUtil.getJedis();
            Set<String> clientSessionIds = jedis.smembers(UMS_CLIENT_SESSION_IDS + "_" + code);
            for (String clientSessionId : clientSessionIds) {
                jedis.del(UMS_CLIENT_SESSION_ID + "_" + clientSessionId);
                jedis.srem(UMS_CLIENT_SESSION_IDS + "_" + code, clientSessionId);
            }
            _log.debug("当前code={}，对应的注册系统个数：{}个", code, jedis.scard(UMS_CLIENT_SESSION_IDS + "_" + code));
            jedis.close();
            // 维护会话id列表，提供会话分页管理
            RedisUtil.lrem(UMS_SERVER_SESSION_IDS, 1, sessionId);
        }
        // 删除session
        RedisUtil.remove(UMS_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    public Map getActiveSessions(int offset, int limit) {
        ...
        for (String id : ids) {
            ...
            // 过滤redis过期session
            if (null == session) {
                RedisUtil.lrem(UMS_SERVER_SESSION_IDS, 1, id);
                total = total - 1;
                continue;
            }
    ...
    public int forceout(String ids) {
        ...
        for (String sessionId : sessionIds) {
            ...
            RedisUtil.set(UMS_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), (int) upmsSession.getTimeout() / 1000);
        }
